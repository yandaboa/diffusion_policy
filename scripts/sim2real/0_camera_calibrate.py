...
<<<<<<< HEAD
    if args.serial and args.camera == "realsense":
        ...
        def _find_dev():
            ...
            d = next((dv for dv, s in zip(devs, found) if s == str(args.serial)), None)
            if d is None:
                raise SystemExit(f"--serial {args.serial} not among connected realsense cameras {found}")
            return d

        def _warmup(c, want=10, budget=8.0):
            """Drop initial frames so the D4xx stream settles (it stalls right after start,
            esp. following an unclean Ctrl-C). Returns how many frames arrived."""
            t0, got = time.time(), 0
            while time.time() - t0 < budget and got < want:
                try:
                    c._pipeline.wait_for_frames(1000)
                    got += 1
                except Exception:
                    pass
            return got
        ...
        if _warmup(cam) == 0:                       # hard stall -> hardware_reset + retry once
            print("[calib] RealSense delivered no frames; hardware_reset + retry...")
            try:
                cam.disable_camera()
            except Exception:
                pass
            dev.hardware_reset()
            time.sleep(5.0)
            cam = RealSenseCamera(_find_dev(), rgb=True, depth=True, align="color")
            if _warmup(cam) == 0:
