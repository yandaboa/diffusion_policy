...
def probe_recorded_action_dim(output_dir):
    """Width of the `action` array already recorded in ``output_dir``, or None.

    Returns None when there is no buffer yet (fresh run) or it has no actions.
    """
    zarray = pathlib.Path(output_dir) / 'replay_buffer.zarr' / 'data' / 'action' / '.zarray'
    if not zarray.is_file():
        return None
    try:
        shape = json.loads(zarray.read_text())['shape']
...
@click.option('--z_terminate', default=0.5, type=float,
...
def main(input, output, robot_ip, match_dataset, match_episode,
    ...
    # The recording buffer in --output is appended to across runs, and its `action` array
    # is fixed at the width of whatever policy created it. Evaluating a checkpoint with a
    # different action layout into the same directory can only fail at the end of the first
    # episode (after a full rollout), so check it up front instead.
    existing_action_dim = probe_recorded_action_dim(output)
    if existing_action_dim is not None and existing_action_dim != action_dim:
        raise click.ClickException(
            f"'{output}' already holds recordings with action_dim={existing_action_dim}, "
            f"but this checkpoint has action_dim={action_dim} "
            f"({arm_action_dim} arm + 1 gripper). Episodes cannot be appended to that "
            f"buffer -- pass a fresh -o/--output directory.")
            ...
            while True:
                ...
                try:
                    ...
                    try:
                        ...
                        # Last-chance cleanup path: the video writers below must still close.
                        print(f"end_episode failed during shutdown: {end_e}")
                    if save_video and episode_video_writer is not None:
                        episode_video_writer.close()
                        episode_video_writer = None
                        print(f"  Episode video saved.")
